...
grep 'ref=cc969c74173950bcf1a047170f30ef0455e9ee3e' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=cc969c74173950bcf1a047170f30ef0455e9ee3e[a-z0-9]*/${root}\/\2/g" "${file}"
done
