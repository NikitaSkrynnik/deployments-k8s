...
grep 'ref=428b292c794369286fb7ffdf5c306a106492ed0c' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=428b292c794369286fb7ffdf5c306a106492ed0c[a-z0-9]*/${root}\/\2/g" "${file}"
done
